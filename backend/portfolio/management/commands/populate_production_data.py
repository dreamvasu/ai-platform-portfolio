...
class Command(BaseCommand):
    help = 'Populates the database with production data (tech stack, journey, projects)'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"✅ Created {len(tech_stack)} technologies")
        ...
        for j_data in journeys:
            ...
            self.stdout.write(f"  ✅ Hour {journey.hour}: {journey.title}")

        self.stdout.write(f"\n✅ Created {len(journeys)} journey entries")

        # === BASE PROJECTS ===
        self.stdout.write("\n🚀 Creating Base Projects...")
        ...
        self.stdout.write(f"\n✅ Created {len(projects)} base projects")

        # === CASE STUDY PROJECTS ===
        self.stdout.write("\n🎯 Creating Case Study Projects...")

        # Ringlet Project
        ringlet_tech = TechStack.objects.filter(name__in=[
            'Django', 'Kubernetes', 'Docker', 'PostgreSQL'
        ])
        ringlet = Project.objects.create(
        ...
        ringlet.tech_stack.set(ringlet_tech)
        self.stdout.write("  ✅ Ringlet - Educational Platform on Kubernetes")

        # Calibra Project
        calibra_tech = TechStack.objects.filter(name__in=[
            'Django', 'Django REST Framework', 'PostgreSQL', 'Terraform', 'Docker'
        ])
        calibra = Project.objects.create(
        ...
        calibra.tech_stack.set(calibra_tech)
        self.stdout.write("  ✅ Calibra - Enterprise Calibration Platform")

        self.stdout.write(f"\n✅ Created 2 case study projects")
        ...
        self.stdout.write(f"\n📊 Summary:")
        self.stdout.write(f"   - Tech Stack: {TechStack.objects.count()} items")
        self.stdout.write(f"   - Journey: {JourneyEntry.objects.count()} entries")
        self.stdout.write(f"   - Projects: {Project.objects.count()} items")
